...
function parseTrainStyleMappingAssetText(text: string): TrainStyleMappingAssets {
    ...
    for (const [rawKey, rawValue] of Object.entries(raw as Record<string, unknown>)) {
        ...
        assert(typeof rawValue === 'string', `mapping value for ${rawKey} must be a string`);

        const normalizedKey = rawKey.trim();
        const normalizedValue = rawValue.trim();
        assert(normalizedValue.length > 0, `mapping value for ${rawKey} must be non-empty`);

        if (mappings.has(normalizedKey)) {
            logger.warn(`duplicate_train_style_mapping key=${normalizedKey} strategy=first_record`);
            continue;
        }
