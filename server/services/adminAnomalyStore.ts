...
export async function scanDailyAnomalies(
    ...
    const rows = listDailyRecordsAll(
        dayStartAt,
        dayStartAt + DAY_SECONDS - 1
    ).sort(sortRoutesAscending);
    const rowsByTrainCode = new Map<string, DailyEmuRouteRow[]>();
    const rowsByEmuId = new Map<number, DailyEmuRouteRow[]>();

    for (const row of rows) {
        const trainKey = trainCodeKey(row.train_code);
        const trainGroup = rowsByTrainCode.get(trainKey);
        if (trainGroup) {
            trainGroup.push(row);
        } else {
            rowsByTrainCode.set(trainKey, [row]);
        }

        const emuId = Number(row.emu_id);
        const emuGroup = rowsByEmuId.get(emuId);
        if (emuGroup) {
            emuGroup.push(row);
        } else {
            rowsByEmuId.set(emuId, [row]);
        }
    }
    ...
    for (const [trainKey, trainRows] of rowsByTrainCode.entries()) {
        const trainParts = trainRows[0]!.train_code;
        const trainCode = formatExternalTrainCode(trainParts);
        const uniqueEmuIds = Array.from(
            new Set(trainRows.map((row) => Number(row.emu_id)))
        ).sort((left, right) => left - right);
        const uniqueEmuCodes = uniqueEmuIds.map((emuId) =>
            formatExternalEmuCode(emuId as EmuId)
        );
    ...
    for (const [emuId, emuRows] of rowsByEmuId.entries()) {
        ...
        const emuCode = formatExternalEmuCode(emuId as EmuId);
        items.push({
            ...
            trainCodes: [formatExternalTrainCode(route.train_code)],
            emuCodes: [emuCode],
            durationSeconds,
