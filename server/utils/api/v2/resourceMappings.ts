...
export interface V2HistoricalTimetableContentMapping extends V2HistoricalTimetableSummaryMapping {
    timetableId: number;
    stops: Array<{
        stationNo: number;
        stationName: string;
        arriveOffset?: number;
        departOffset?: number;
        stationTrainCode: { prefix: string; number: number };
        isStart: boolean;
        isEnd: boolean;
    }>;
}
...
export function resolveTimetableContentMappings(
    ids: readonly number[]
): Record<string, V2HistoricalTimetableContentMapping> | undefined {
    const uniqueIds = [...new Set(ids)].filter((id) => Number.isInteger(id));
    uniqueIds.sort((left, right) => left - right);

    const mappings: Record<string, V2HistoricalTimetableContentMapping> = {};
    let foundAny = false;
    ...
    for (const id of uniqueIds) {
        ...
        if (content === null) {
            console.error(
                `v2 timetable content mapping missing timetableId=${id}`
            );
            continue;
        }
