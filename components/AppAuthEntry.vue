<template>
    <template v-if="shouldShowEntry">
        <div class="relative z-30 px-4 pt-4 sm:hidden">
            <div class="mx-auto flex w-full max-w-7xl justify-end">
                <div
                    class="inline-flex flex-wrap items-center justify-end gap-2 rounded-[1.75rem] border border-white/55 bg-white/60 p-2 shadow-[0_20px_48px_-28px_rgba(15,23,42,0.28)] ring-1 ring-slate-200/45 backdrop-blur-md">
                    <NuxtLink
                        to="/docs"
                        class="inline-flex items-center gap-2 rounded-full border border-slate-200/70 bg-white/90 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-white hover:text-slate-950">
                        文档
                    </NuxtLink>
                    <NuxtLink
                        v-if="
