...
export const oauthFlowOverview = [
    '先登录 Open CRH Tracker 账号，在用户页的“开发”选项卡里创建一个 OAuth 客户端，登记回调地址并申请需要的 scope。',
    '普通用户的 scope 需要等待管理员审核通过后才能授权；客户端创建者可以在自己的 scope 仍处于待审核状态时，先完成登录和授权测试。',
    '用户在浏览器里完成登录与授权确认后，应用会在回调地址收到一个一次性 authorization code。',
    '应用拿着 code 和当初的 PKCE verifier，调用 /oauth/token 换取 access_token 和 id_token，之后按 Bearer 方式访问 /oauth/userinfo。'
];

const createClientRequestExample = [
    'curl -X POST /api/v2/oauth/clients \\',
...
export const oauthDocsSections: DocsContentSection[] = [
    {
        ...
        title: '这份授权是怎么工作的',
        summary:
            '简单来说，整个过程只做三件事：创建客户端、发起授权、交换令牌。下面先讲清楚每一步在做什么。',
        blocks: [
            {
                type: 'list',
                title: '当前能力',
                items: [
                    ...
                    'PKCE 是必填项，code_challenge_method 当前只支持 S256。',
                    '令牌交换成功后返回 access_token、id_token、expires_in 和 scope。',
                    '目前没有 refresh_token；access_token 有效期约 30 天（2592000 秒），过期后需要重新发起授权流程。'
                ]
            },
            {
                type: 'field-cards',
                ...
                cards: [
                    {
                        ...
                        required: true,
                        description:
                            '浏览器跳转入口。用户会在这里完成登录、授权确认，并最终被重定向到你注册过的 redirect_uri。'
                    },
                    {
                        path: '/oauth/token',
                        valueType: 'POST',
                        required: true,
                        description:
                            '授权码交换端点。客户端提交 authorization code 与 code_verifier 后，在这里换取 access_token 和 id_token。'
                    },
                    {
                        path: '/oauth/userinfo',
                        valueType: 'GET',
                        required: true,
                        description:
                            '用户信息端点。应用带上 access_token 后可在这里读取 OIDC sub；如果 scope 允许，还会返回 preferred_username。'
                    }
                ]
            },
            {
    ...
    },
    {
        ...
        title: '先创建 OAuth 客户端',
        summary:
            '创建客户端是在网页上完成的，但如果你更习惯用接口，也可以直接调用 v2 的客户端接口。',
        blocks: [
            {
                type: 'paragraph',
                text: '先登录站内账号，然后点击右上角用户名进入用户页，选择开发选项卡，切换到 OAuth 面板，填写信息创建 OAuth 客户端。'
            },
            {
                type: 'field-cards',
                title: '创建时要准备的信息',
                cards: [
                    ...
                    },
                    {
                        ...
                        description: '可选描述，便于用户理解这个应用的用途。'
                    },
                    {
                        path: 'homepageUrl',
                        valueType: 'string | null',
            ...
            },
            {
                ...
                title: '用 v2 接口创建客户端',
                language: 'bash',
                code: createClientRequestExample
            },
            {
                type: 'paragraph',
                text: '客户端创建成功后，普通用户需要等待管理员审核 scope；客户端创建者可以在自己客户端的 scope 仍为待审核时继续完成 OAuth 登录与授权测试。'
            }
        ]
    },
    {
        ...
        eyebrow: '发起授权',
        title: '发起授权请求',
        summary: '授权请求必须在浏览器里发起，让用户自己完成登录和确认。',
        blocks: [
            {
                type: 'field-cards',
                title: '授权请求参数',
    ...
    },
    {
        ...
        blocks: [
            ...
            },
            {
                ...
                text: '服务端会从请求体读取这些字段。建议使用 application/x-www-form-urlencoded 提交，这是 OAuth 2.0 中 token 交换最常见的提交方式。',
                cards: [
                    {
                        path: 'grant_type',
                        valueType: 'string',
            ...
            {
                type: 'paragraph',
                text: '注意 expires_in 的单位是秒：2592000 秒约等于 30 天。access_token 和 id_token 的有效期都是这个时长；由于没有 refresh_token，过期后需要让用户重新走一遍授权流程。'
            }
        ]
    },
    {
        ...
        blocks: [
            {
                ...
                items: [
                    ...
                    '没有 refresh_token，access_token 约 30 天过期，过期后需要重新授权。',
